...
echo -e "\n===== Step 3: Preparing for build ====="

# Fix setup.py to skip LLVM download
echo "Modifying setup.py to skip LLVM download..."
if [ -f "scripts/fix_setup_py.py" ]; then
  python scripts/fix_setup_py.py
  echo "Setup.py modified successfully."
else
  echo "Warning: fix_setup_py.py script not found. Using existing setup.py."
fi

# Step 4: Clean and build the source distribution only (sdist)
...
export TRITON_WHEEL_VERSION_SUFFIX="rc1"
...
# Step 5: Verify the built packages
echo -e "\n===== Step 5: Verifying the built packages ====="
...
echo -e "\n===== Step 6: Uploading directly to PyPI ====="
